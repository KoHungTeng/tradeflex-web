...
export async function POST(request: NextRequest) {
  ...
  // 檢查用戶方案，免費版限制 50 筆已完成交易
  const { data: planData } = await supabase.from('user_plans').select('plan').eq('user_id', user.id).single()
  const plan = planData?.plan || 'free'
  if (plan === 'free') {
    const { count } = await supabase.from('completed_trades').select('*', { count: 'exact', head: true }).eq('user_id', user.id)
    if ((count || 0) >= 50) {
